...
import static net.runelite.client.plugins.externals.presetmanager.PresetConverter.*;
import static net.runelite.client.plugins.externals.presetmanager.PresetRenderEngine.*;
import static net.runelite.client.plugins.externals.presetmanager.WidgetUtils.*;
...
public class PresetManager extends Plugin
{
	...
	public void onScriptPostFired(ScriptPostFired event) {
		...
		getEquipmentFromId(presetToView, getPresetFromConfig(config))
	...
	public void init() {
		...
		presetBackgroundWidget = createGraphic(parent,
				"preset-helper",
		...
		for (int preset = 1; preset <= 5; preset++) {
			presetBackgroundWidget.setAction(preset, VIEW_TAB + GET_NAME_FROM_ID.apply(gson.fromJson(config.configBlob(), PresetList.class), preset));
		}

		presetBackgroundWidget.setOnOpListener((JavaScriptCallback) this::handleTagTab);

		presetIconWidget = createGraphic(parent,
				"",
	...
	private void applyLimitAndPossiblyUpdateConfig(int configToUpdate) {
		PresetList presets = getPresetFromConfig(config);
		String name = config.name();

		if (configToUpdate >= 1 && configToUpdate <= 5) {
			config.configBlob(updateConfigWithSnapshot(presets, client, configToUpdate, name));
		} else {
			dispatchError("ERROR! Preset is out of bounds!", client);
		}
	}
	...
	{
		addedWidgets.add(createGraphic(itemContainer,
	...
	public void handlePresetClick(ScriptEvent event)
	{
		...
		// withdraw equipment
		getEquipmentFromId(presetToView, getPresetFromConfig(config)).ifPresent(equipmentPreset -> {
