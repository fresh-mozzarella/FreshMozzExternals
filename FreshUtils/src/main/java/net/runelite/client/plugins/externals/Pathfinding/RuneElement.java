...
import net.runelite.client.plugins.externals.api.EquipmentItem;
...
public enum RuneElement {
    ...
    public int getCount(GameAPI gameAPI) {
        if (haveStaff(gameAPI)) {
            return Integer.MAX_VALUE;
        }
        List<InventoryItem> runes = gameAPI.inventory().withNamePart("rune").all();
        ...
        return items.stream().mapToInt(InventoryItem::quantity).sum() + RunePouch.getQuantity(gameAPI, this);
    }

    private boolean haveStaff(GameAPI gameAPI) {
        EquipmentItem item = gameAPI.equipment().withNamePart("staff").first();
