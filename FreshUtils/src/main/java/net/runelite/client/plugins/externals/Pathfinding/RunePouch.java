...
public class RunePouch {
    ...
    public enum RuneSlot {
        ...
        public int getQuantity(GameAPI gameAPI) {
            return gameAPI.varb(quantityVarbitIndex);
        }

    }

    public static int getQuantity(GameAPI gameAPI, RuneElement runeElement) {
        if (!hasPouch(gameAPI)) {
            return 0;
        }
        ...
        for (RuneSlot slot : RuneSlot.values()) {
            ...
            return slot.getQuantity(gameAPI);
        }

        return 0;
    }

    private static boolean hasPouch(GameAPI gameAPI) {
        return gameAPI.inventory().withName("Rune pouch").exists();
    }
}