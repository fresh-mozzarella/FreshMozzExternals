...
public class TransportLoader {
    ...
    public static List<Transport> buildTransports(GameAPI gameAPI) {
        ...
        // Edgeville
        if (gameAPI.modifiedLevel(Skill.AGILITY) >= 21 && gameAPI.membersWorld()) {
        ...
        // Crabclaw isle
        if (gameAPI.inventory().withId(COINS).quantity() >= 10000) {
        ...
        // Port Sarim
        if (gameAPI.varb(4897) == 0) {
            if (gameAPI.varb(8063) >= 7) { // todo: lower?
        ...
        transports.add(new Transport(
                ...
                gameAPI.varb(6396) >= 105 ? new Position(3629, 9644, 0) : new Position(3626, 9618, 0),
                0, 0,
                g -> {
                    if (g.varb(2590) < 1) {
        ...
        //Spirit Tree's
        if (gameAPI.varp(111) == 9 && gameAPI.membersWorld()) {
            for (var source : SPIRIT_TREES) {
                if (source.location.equals("Gnome Stronghold") && gameAPI.varp(150) < 160) {
                    continue;
                }
...
//        3625 3224 0 3625 3223 0 Open Door 18057
        ...
        var icyeneGraveyardTarget = gameAPI.varb(7255) <= 44 ? new Position(3684, 3181, 0) : new Position(3684, 3176, 0);
