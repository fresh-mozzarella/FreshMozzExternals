...
public class BankLocations {
    ...
    public static void walkToBank(GameAPI gameAPI) {
        ...
        if (gameAPI.varp(302) >= 61) {
            validBanks.add(CANIFIS_BANK);
        }

        if (gameAPI.varb(1990) >= 200) {
            validBanks.add(BURGH_DE_ROTT_BANK);
        }

        new Walking(gameAPI).walkTo(Area.union(validBanks.toArray(new Area[0])));
    }
}
