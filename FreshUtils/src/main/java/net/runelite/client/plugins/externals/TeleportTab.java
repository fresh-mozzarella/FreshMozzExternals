...
public enum TeleportTab {
    ...
    public boolean canUse(GameAPI gameAPI) {
        return hasRequirements(gameAPI) && gameAPI.inventory().withName(this.tabletName).exists();
    }

    public boolean hasRequirements(GameAPI gameAPI) {
        if (this == SALVE_GRAVEYARD_TELEPORT_TAB && gameAPI.varp(302) < 61) {
            return false;
        }
        return this != ARDOUGNE_TELEPORT_TAB || gameAPI.varp(165) >= 30;
    }
}