...
public abstract class iActor implements Locatable, Interactable {
    ...
    public iActor target() {
        ...
        if (interacting instanceof NPC)
            return gameAPI.getFromClientThread(() -> new iNPC(gameAPI(), (NPC) interacting, client().getNpcDefinition(((NPC) interacting).getId())));
        else if (interacting instanceof Player) {
            Player player = (Player) interacting;
            return gameAPI.getFromClientThread(() -> new iPlayer(gameAPI(), player, player.getPlayerComposition()));
        } else
